...
def _col_ref(key: str, promoted_keys: set[str], prefix: str = "") -> tuple[str, bool]:
    """Return (sql_expr_for_value, is_promoted)."""
    if key in promoted_keys and _is_promotable_ident(key):
        return f'{prefix}"{key}"', True
    return f"{prefix}tags['{_sql_escape(key)}']", False
...
def tag_filter_sql(f: TagFilter, promoted_keys: set[str], prefix: str = "") -> str:
    op = f.op
    if f.key_is_regex:
        ...
        return (
            f"{prefix}tags IS NOT NULL AND list_bool_or(list_transform(map_entries({prefix}tags), "
            f"e -> regexp_matches(e.key, '{key_pat}') AND regexp_matches(e.value, '{val_pat}'{flag})))"
        )

    val_expr, promoted = _col_ref(f.key, promoted_keys, prefix)
...
def tag_filters_sql(filters: list[TagFilter], promoted_keys: set[str], prefix: str = "") -> str:
    parts = [tag_filter_sql(f, promoted_keys, prefix) for f in filters]
    if not parts:
        return "TRUE"
