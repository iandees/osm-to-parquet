...
| [Cloudflare R2 Data Catalog](https://developers.cloudflare.com/r2-data-catalog/) + [R2 SQL](https://blog.cloudflare.com/r2-sql-deep-dive/) | Managed Iceberg REST catalog inside an R2 bucket; DuckDB, PyIceberg, Spark can read it; R2 SQL is a serverless engine at $2.50/TB scanned, with joins/CTEs as of 2026. An option if we want Iceberg instead of DuckLake or a hand-rolled manifest. Zero egress on R2 is the headline economic advantage. |
